...
function printTree(node, prefix = '', isLast = true) {
  ...
  console.log(
    prefix + (isLast ? '└── ' : '├── ') + node.data + '(' + node.color[0] + ')'
  );
...
class RBTreeValidator {
  static validate(tree) {
    ...
    return {
      ...
      message: allValid
        ...
        : 'Tree violates Red-Black properties'
    };
  }
  ...
  static checkRule1(node) {
    ...
    if (!validColor) {
      return {
        valid: false,
        message: `Node ${node.data} has invalid color: ${node.color}`
      };
    }
...
class PerformanceTester {
  static testInsertion(TreeClass, data, label) {
    ...
    console.log(
      `  Efficiency: ${(height / Math.log2(data.length)).toFixed(2)}x optimal`
    );
    console.log('');
...
_Remember: Data structures are tools to solve problems. Focus on understanding when and why to use Red-Black Trees, not just how to implement them!_
